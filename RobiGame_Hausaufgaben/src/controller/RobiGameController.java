...
public class RobiGameController extends PApplet{
	 ...
	 */
	public void setup() {
		spielerName = JOptionPane.showInputDialog("Bitte Namen eingeben");
		...
		tim.scheduleAtFixedRate(tTask,new Date(), 50);
	}
	...
	private void drawGameOver() {
		fill(255);
		textSize(42);
		text("Game Over:" + " " + "Winner", 100, 400);
		float posH = 50;
		
		for (int i = 0; i <= 10; i++); {
			for (HighScore scores : highscore.getHighscoreList()) {
				textSize(15);
				text(scores.getName() + " " + ":" + " " + scores.getScore(), 500, posH);
				posH = posH + 50;
			}
		}
	...
	private boolean checkScore(Robi r) {
		if (r.getScore() >= 100) {
			...
			highscore.addHighscores(new HighScore(spielerName, playerRobi.getScore()));
